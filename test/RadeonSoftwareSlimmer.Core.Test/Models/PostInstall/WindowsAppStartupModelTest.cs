    ...
    public class WindowsAppStartupModelTest
    {
        ...
        public void Disable_SetsDisabledKeys()
        {
            ...
            using (Assert.EnterMultipleScope())
            {
                ...
                Assert.That(dummyRegistry.MockCurrentUser.GetTestSubKey(RSX_LAUNCHER_REG_PATH).Values.ContainsKey(RSX_LAUNCHER_REG_STATUS_NAME), Is.True);
                Assert.That(dummyRegistry.MockCurrentUser.GetTestSubKey(RSX_LAUNCHER_REG_PATH).Values[RSX_LAUNCHER_REG_STATUS_NAME].Value, Is.EqualTo(STATE_DISABLED_VALUE));
                Assert.That(dummyRegistry.MockCurrentUser.GetTestSubKey(RSX_LAUNCHER_REG_PATH).Values[RSX_LAUNCHER_REG_STATUS_NAME].Kind, Is.EqualTo(CoreRegistryValueKind.DWord));

                Assert.That(dummyRegistry.MockCurrentUser.GetTestSubKey(RSX_LAUNCHER_REG_PATH).Values.ContainsKey(RSX_LAUNCHER_REG_STARTUPONCE_NAME), Is.True);
                Assert.That(dummyRegistry.MockCurrentUser.GetTestSubKey(RSX_LAUNCHER_REG_PATH).Values[RSX_LAUNCHER_REG_STARTUPONCE_NAME].Value, Is.EqualTo(STATE_STARTUPONCE_YES));
                Assert.That(dummyRegistry.MockCurrentUser.GetTestSubKey(RSX_LAUNCHER_REG_PATH).Values[RSX_LAUNCHER_REG_STARTUPONCE_NAME].Kind, Is.EqualTo(CoreRegistryValueKind.DWord));
