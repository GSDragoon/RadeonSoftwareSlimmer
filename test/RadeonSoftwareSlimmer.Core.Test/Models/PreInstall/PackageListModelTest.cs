...
namespace RadeonSoftwareSlimmer.Core.Test.Models.PreInstall
{
    ...
    public class PackageListModelTest
    {
        ...
        {
            PackageListModel packageList = new PackageListModel(_fileSystem, _logger);
            _fileSystem.AddFile(Path.Combine(InstallRoot, "Bin64", "cccmanifest_64.json"), new MockFileData(File.ReadAllText(Path.Combine(_testDataDirectory, "PackageModel_cccmanifest.json"))));
            _fileSystem.AddFile(Path.Combine(InstallRoot, "Config", "InstallManifest.json"), new MockFileData(File.ReadAllText(Path.Combine(_testDataDirectory, "PackageModel_installmanifest.json"))));
        ...
        {
            PackageListModel packageList = new PackageListModel(_fileSystem, _logger);
            MockFileData ccmanifesData = new MockFileData(File.ReadAllText(Path.Combine(_testDataDirectory, "PackageModel_cccmanifest.json")));
            MockFileData installmanifestData = new MockFileData(File.ReadAllText(Path.Combine(_testDataDirectory, "PackageModel_installmanifest.json")));
            _fileSystem.AddFile(Path.Combine(InstallRoot, "Bin64", "cccmanifest_64.json"), ccmanifesData);
            _fileSystem.AddFile(Path.Combine(InstallRoot, "Config", "InstallManifest.json"), installmanifestData);
            IDirectoryInfo installerDir = _fileSystem.DirectoryInfo.New(InstallRoot);
        ...
        public void LoadOrRefresh_DoesNotReplaceExistingBackupFiles()
        {
            ...
            _fileSystem.AddFile(Path.Combine(InstallRoot, "Bin64", "cccmanifest_64.json"), new MockFileData(File.ReadAllText(Path.Combine(_testDataDirectory, "PackageModel_cccmanifest.json"))));
            _fileSystem.AddFile(Path.Combine(InstallRoot, "Config", "InstallManifest.json"), new MockFileData(File.ReadAllText(Path.Combine(_testDataDirectory, "PackageModel_installmanifest.json"))));
            _fileSystem.AddFile(Path.Combine(InstallRoot, "RSS_Backup", "Packages", "Bin64", "cccmanifest_64.json"), new MockFileData(File.ReadAllText(Path.Combine(_testDataDirectory, "PackageModel_cccmanifest.json"))));
            _fileSystem.AddFile(Path.Combine(InstallRoot, "RSS_Backup", "Packages", "Config", "InstallManifest.json"), new MockFileData(File.ReadAllText(Path.Combine(_testDataDirectory, "PackageModel_installmanifest.json"))));
            IDirectoryInfo installerDir = _fileSystem.DirectoryInfo.New(InstallRoot);
            MockFileData ccmanifestOrig = _fileSystem.GetFile(Path.Combine(InstallRoot, "RSS_Backup", "Packages", "Bin64", "cccmanifest_64.json"));
            MockFileData installmanifestOrig = _fileSystem.GetFile(Path.Combine(InstallRoot, "RSS_Backup", "Packages", "Config", "InstallManifest.json"));
        ...
        public void RemovePackage_PackageIsNull_ThrowsArgumentNullException()
        {
            ...
            Assert.That((System.Action)(() => packageList.RemovePackage(null)), Throws.ArgumentNullException);
        }
        ...
        public void RemovePackage_PackageDoesNotExist_DoesNotRemove()
        {
            ...
            PackageModel packageToRemove = new PackageModel(_fileSystem.FileInfo.New(Path.Combine(installerDir.FullName, "Bin64", "cccmanifest_64.json")))
            {
        ...
        public void RestoreToDefault_RestoresPackageFilesFromBackup()
        {
            ...
            _fileSystem.AddFile(Path.Combine(InstallRoot, "RSS_Backup", "Packages", "Config", "InstallManifest.json"), new MockFileData(File.ReadAllText(Path.Combine(_testDataDirectory, "PackageModel_installmanifest.json"))));
            IDirectoryInfo installerDir = _fileSystem.DirectoryInfo.New(InstallRoot);
            _fileSystem.GetFile(Path.Combine(InstallRoot, "RSS_Backup", "Packages", "Bin64", "cccmanifest_64.json")).TextContents = "Different data to simulate modifying the file";
            _fileSystem.GetFile(Path.Combine(InstallRoot, "RSS_Backup", "Packages", "Config", "InstallManifest.json")).TextContents = "Different data to simulate modifying the file";
            packageList.LoadOrRefresh(installerDir);

            packageList.RestoreToDefault();

            MockFileData ccmanifest = _fileSystem.GetFile(Path.Combine(InstallRoot, "Bin64", "cccmanifest_64.json"));
        ...
        private List<PackageModel> ExpectedLoadedPackageModel(string installerRoot)
        {
            ...
            {
                new PackageModel(_fileSystem.FileInfo.New(Path.Combine(installerRoot, "Config", "InstallManifest.json")))
                {
                ...
                new PackageModel(_fileSystem.FileInfo.New(Path.Combine(installerRoot, "Bin64", "cccmanifest_64.json")))
                {
                Description = "Test Description",
                ProductName = "Test productName",
                Url = @"\Parent\child\file.extension",
